...
page_view <- data_1 %>% filter(eventName == "page_view") %>% group_by(date, month, year, eventName) %>% summarize(eventCount = sum(eventCount), activeUsers = sum(activeUsers)) %>% mutate(eventCountPerUser = eventCount / activeUsers) %>% 
  arrange(date, desc(eventCount)) %>% bind_rows(wix_page_view) %>% ungroup() %>% bind_rows(recovery_pageview) %>% ungroup() %>% arrange(date) %>% group_by(date, month, year, eventName) %>% summarise(eventCount = sum(eventCount)) %>% ungroup() %>% mutate(moving.average = ceiling(runMean(.$eventCount)))
...
imput_page_view_unique <- imput_page_view_unique %>% mutate(moving.average = ceiling(runMean(imput_page_view_unique$eventCount)))

...
xts_imput_page_view_unique <- read_sheet(ss, sheet = "imput_page_view_unique") %>% column_to_rownames(var = "date") %>% select(moving.average) %>% as.xts()
```
...
## Daily Plot (10-day Moving Average)

```{r daily plot, out.width='100%', echo=FALSE}
highchart(type = "stock") %>% 
  hc_add_series(xts_page_view, yAxis = 0, name = "人次") %>% 
  hc_add_series(xts_imput_page_view_unique, yAxis = 0, name = "人數")
```